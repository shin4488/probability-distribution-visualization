...
- **SEO**: index.html carries a keyword-rich meta description, a canonical link to the app root (shared query-string URLs must not count as duplicate content), Open Graph/Twitter-card tags with a generated 1200x630 OGP image, JSON-LD WebApplication structured data, and a noscript fallback; robots.txt and sitemap.xml are served from public/
