...
- **Chart ordering**: order the charts the way a statistics learner studies them. If one distribution presupposes another, show the prerequisite first, and show closely related distributions near each other. Among the discrete distributions the order is fixed as Bernoulli → binomial → Poisson → geometric → negative binomial, and because the negative binomial is derived from Poisson and gamma it comes after both (full order: Bernoulli → binomial → Poisson → geometric → exponential → gamma → negative binomial → beta → normal → log-normal)
