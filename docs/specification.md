...
- **Case guidance**: show a schematic illustration, a candidate distribution, its assumptions and limitations, and a link to an alternative case. Links open the existing chart page with only the chosen distribution visible in a wider, centered card, preserve its current parameters, and return to the case through the main navigation (no extra context banner on the chart page).
