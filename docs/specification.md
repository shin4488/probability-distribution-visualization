...
- **Shared Git skills**: `create-branch` and `post-merge` come from the shared agent plugin. They do not specify a model; model selection stays with the user's tool configuration.
...
- **SEO**: index.html carries a keyword-rich meta description, a canonical link to the app root (shared query-string URLs must not count as duplicate content), Open Graph/Twitter-card tags with a generated 1200x630 OGP image, JSON-LD WebApplication structured data, and a noscript fallback; robots.txt and sitemap.xml are served from public/. Search Console ownership is verified via the HTML-tag method (the GA method cannot match this site's guarded, dynamically injected gtag loader). For English-locale reach (lightweight approach): hreflang alternates point to `?lang=ja` / `?lang=en` with an x-default, `document.title` and the meta description follow the active locale in the rendered DOM, and og:description is bilingual since social crawlers do not run JS
- **Google Analytics 4** measures page views, loaded only on the production hostname so development traffic never pollutes the metrics (measurement ID is a placeholder until the GA property is created). GA4's enhanced measurement "page changes based on browser history events" should stay off — the app rewrites the URL via replaceState on every parameter change. Custom events: `param_change` (per distribution, debounced so one continuous drag counts once), `histogram_on` (per distribution, fired only when turning the sample simulation on), `help_open` (once per page load), `lang_switch`, and `theme_switch`
- **User feedback**: a permanent, unobtrusive footer widget asks "Was this site helpful?" with a one-click 👍/👎 (sent as the GA4 `feedback_rating` event — anonymous, production only; the answer is remembered per browser and the question reappears one day later, so repeat visitors are not nagged but rating changes can still be tracked), plus a link to a Google Form for free-text requests (form URL is a placeholder until the form exists; the link stays hidden while it is a placeholder). Free-text goes to a form because the static site has no backend of its own
...
- **Visual use-case finder**: add a separate `?page=guide` page, reached through the “Find by use case” navigation. Beginners choose from five illustrated types of quantities, then concrete situations; no text search or automatic fitting is used. Ten cases cover the existing distributions.
- **Case guidance**: show a schematic illustration, a candidate distribution, its assumptions and limitations, parameter guidance, and a link to an alternative case. Illustrations are explicitly illustrative. Links open the existing chart page with only the chosen distribution visible in a wider, centered card, preserve its current parameters, and return to the case through the main navigation (no extra context banner on the chart page).
- **Guide sharing and accessibility**: `page=guide`, `category`, and `case` are synchronized with replaceState; defaults are omitted and invalid values fall back safely. Page navigation uses relative links so direct visits, refresh, and browser back work on GitHub Pages. Both languages/themes, responsive layouts, keyboard selection and focus transfer to the next step after category/case selection are supported.

- **Review refinements**: parameter controls wrap their label above a usable slider in narrow cards, based on container width. Accent foreground/background pairs remain readable in both themes. The chart action sits at the bottom of the distribution card on the left; the explanations on the right use concrete, plain language.
- **Explanation boundaries**: gamma task-count explanations require positive integer shape and independent equal-rate exponential durations; fractional shapes remain valid. Beta shapes are not rounded into alleged observed counts; explain success/failure updates from an explicit uniform prior. Each chart has a localized accessible name with its current parameters.
- **Concise guide copy**: remove generic reminders below the chart action and at the bottom of the guide; retain the conditions and cautions specific to each case. Omit the introductory paragraph, category instruction paragraph, and category/count summary above the cases. Reference links are always visible without a disclosure control; the heading does not include a language suffix.
- **Meaningful analytics**: measure content views separately from settings changes, and record guide navigation, actual graph exploration, simulation use, and feedback. Send no numerical parameter values; debounce continuous changes and flush pending events on exit. Register GA4 dimensions and a monitoring dashboard; define metrics as observed behavior, not proof of learning. See [measurement guide](analytics.md).

- **Self-contained case details**: keep introductory explanations inside the distribution card, without repeating the list summary above it. Omit chart-control instructions and the illustration disclaimer from case details. Use concrete examples for assumptions and limitations. On desktop, place the back button beside the detail heading; stack them on small screens.
