...
- **Mechanical skills run on the cheaper model**: `create-branch` and `post-merge` declare `model: sonnet` (the unversioned alias, so it always tracks the latest available Sonnet) since they are routine procedures that do not need the session's stronger model
