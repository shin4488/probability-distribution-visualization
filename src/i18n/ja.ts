...
export const ja = {
  ...
  'dist.normal.usecase':
    '平均{mu}点・標準偏差{sigma}点のテストなら、約68%の人が{lo1}〜{hi1}点、約95%が{lo2}〜{hi2}点に収まります。点数が釣鐘型になるのは、1問ごとの正解・不正解という小さな要因が多数足し合わさるためです(中心極限定理)。同じ理屈で、1件ずつは歪んだ分布の売上でも、1日の合計売上は正規分布に近づきます。だから「明日の売上はだいたい平均±2σの範囲」という形で見積もれます。品質管理の管理限界(±3σ)も同じ考え方で、正規分布では99.7%が平均±3σに収まるため、そこを外れた値は偶然のばらつきではなく異常のサインと判断できます。',
